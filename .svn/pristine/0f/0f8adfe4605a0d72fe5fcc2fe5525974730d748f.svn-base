        ...
        try {
            ...
            for (int intCounter = intStartRow; intCounter <= intEndRow; intCounter++) {
                try {
                    ...
                    boolean error = memeber.verifyErrorMessageForUnavailableService();
                    Thread.sleep(15000);
                    ...
                    assertTrue(memeber.memberName().contains(data.get("ValidLastName")), "Result is matched with the name entered");
                    String fullName = memeber.memberName();
                    String fullNameSplit[] = fullName.split(",");
                    String lastname = fullNameSplit[0];
                    assertTrue(lastname.startsWith(data.get("ValidLastName")), "Last name is Wildcarded");
                    memeber.memberNumClick();
                    //verify member fields are auto populated
                    verifyMemberDetailsPopulated();
                    home.leaveButton();
                    ...
                    String splitExcelName[] = data.get("LastNameFirstName").split(",");
                    String lastNameFromExcel = splitExcelName[0].trim();
                    String firstNameFromExcel = splitExcelName[1].trim();
                    assertTrue(memeber.memberName().equalsIgnoreCase(data.get("LastNameFirstName")), "Results returned the match");

                    //verify last name is wild carded
                    String firstNameAndLastName = memeber.memberName();
                    String nameSplit[] = firstNameAndLastName.split(",");
                    String lastName = nameSplit[0].trim();
                    String firstName = nameSplit[1].trim();
                    assertTrue(lastName.startsWith(lastNameFromExcel) && firstName.startsWith(firstNameFromExcel), "FirstName and LastName are Wildcarded");
                    memeber.memberNumClick();
                    //verify member fields are auto populated
                    verifyMemberDetailsPopulated();
                    home.leaveButton();
