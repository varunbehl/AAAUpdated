...
public class TC_CRERSCallLockSmith extends LoginLib
{
	String callId;
...
//String text = dataBean.getValue(,"text");
    		...
    		for(int intCounter=intStartRow;intCounter<=intEndRow;intCounter++)
    		{
    			try {
    				fnOpenTest();
		                ...
		                this.reporter.initTestCaseDescription("1597: TC-ERS Call Locksmith"+ " From Iteration " + StartRow + " to " + EndRow );
    	...
    	try
    	{
    		...
    		for(int intCounter=intStartRow;intCounter<=intEndRow;intCounter++)
    		{
    			try {
					...
					veh.selectpaceSettercode(data.get("Pacesettercode"));
					veh.scriptErrorHandle();
					veh.handleScriptErrorInVehicleTriagePage();
					//validate pacesettercode
					veh.validatePacesetterCode(dataBean.getValue(data,"Pacesettercode") );
	...
	@Parameters({"StartRow","EndRow","nextTestJoin"})
	@Test()
	public void createCallWithSpecialEntitlementLockoutHLOCK
			(int StartRow,String EndRow,boolean nextTestJoin) throws Throwable {
		try
		{
			int intStartRow=StartRow;
			int intEndRow=ReportControl.fnGetEndRowCunt(EndRow, "createCallWithSpecialEntitlementLockoutHLOCK", TestData, "D3CallRecieving");
			for(int intCounter=intStartRow;intCounter<=intEndRow;intCounter++)
			{
				try {
					fnOpenTest();
					ReportStatus.fnDefaultReportStatus();
					ReportControl.intRowCount=intCounter;
					Hashtable<String, String> data=TestUtil.getDataByRowNo("createCallWithSpecialEntitlementLockoutHLOCK", TestData, "D3CallRecieving",intCounter);
					...
					mem.memberNumClick();
					mem.memberLevelVerification();
					home.clickMmbSplEntitlementLink();
					home.selectHLOCKMmbSplEntitlementLinkChkBox();
					home.clickHLOCKMmbSplEntitlementSaveBtn();
					String SplEntitleCode=home.getTxtOfSplEntitlementCODEMmrTab();
					assertTrue(SplEntitleCode.equalsIgnoreCase(": HLOCK"), "Verify HLOCK entitlement");
	...
	public void validateAutoSpotanCallisSaved(Hashtable<String, String> data) throws Throwable {
		...
		assertTrue(serviceTypeOnServiceTab, "Verify Service type info is Listed");
		dih.logOut();
	}
}